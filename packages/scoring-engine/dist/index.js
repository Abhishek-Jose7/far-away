...
exports.calculateHealthAndPrediction = calculateHealthAndPrediction;
/**
 * Agent 2: Verification Agent Logic
 * Calculates a confidence score for reports based on redundancy and user trust.
 */
function calculateConfidenceScore(recentReportsCount, reporterRole) {
 ...
 */
function calculateHealthAndPrediction(assetType, lastMaintenanceDateStr, reports) {
    const now = new Date();
    ...
    // health = 100 - maintenancePenalty - reportPenalty - confidencePenalty
    const healthScore = Math.max(0, Math.round(100 - maintenancePenalty - cappedReportPenalty - cappedConfidencePenalty));
    ...
    let predictedFailureHours = null;
    let status = 'healthy';
    if (healthScore >= 85) {
        status = 'healthy';
